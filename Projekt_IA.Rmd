...
```{r}
# Dane dla portfeli jednoelementowych
asset_names <- c("CCC", "VNQ", "OrangeJuice", "LVMH")
portfolio_risk <- c(s1, s2, s3, s4)            # Ryzyko (SD)
portfolio_return <- c(mean(stopyCCC), mean(stopyVNQ), mean(stopy_sok), mean(stopyLVMH))  # Stopa zwrotu (Mean)
portfolio_efficiency <- portfolio_return / portfolio_risk  # Efektywność (Sharpe, RF = 0)
...
# bp zawiera pozycje środkowe słupków
text(
  x = as.vector(bp),  # Pozycje środkowe słupków
  y = as.vector(portfolio_data),  # Wartości danych
  labels = paste0(round(as.vector(portfolio_data), 2), "%"),  # Etykiety (zaokrąglone wartości)
  pos = ifelse(as.vector(portfolio_data) >= 0, 3, 1),  # Umiejscowienie nad/poniżej słupka
  cex = 0.8, col = "black"
)
